  ...
  return (
    <div className="group bg-[var(--color-blush)] rounded-2xl p-7 border border-[#f7ddd5] hover:border-[#d8a4af]/60 transition-all duration-300 hover:shadow-lg hover:shadow-[#d8a4af]/10 hover:-translate-y-1">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div>
          <p className="font-body text-[14px] tracking-[0.25em] uppercase text-[#6b403b] mb-1.5">
            {item.period}
          </p>
        ...
        <span className="font-body text-[14px] tracking-widest text-[#7d5a56] bg-[#f7ddd5]/60 rounded-full px-2.5 py-1 shrink-0">
          {item.location.split(",")[1]?.trim() || item.location}
        </span>
      </div>
      <p className="font-body font-400 text-[#7d5a56] text-sm mb-3">
        {item.degree}
      </p>
      <p className="font-body font-300 text-[#7d5a56] text-xs italic">
        {item.note}
      </p>
    </div>
  );
